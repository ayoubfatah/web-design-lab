      ...
      {showSidebar && (
        <div className="border-r-[1px] h-full w-full">
          <ul>
            {sideBarShowing === 1 && (
              ...
              >
                {navItems.map((item, index) => (
                  <li
                    key={index}
                    className="flex px-7 gap-2   p-2 transition-colors"
                  >
              ...
              <div className={` px-3 py-5 `}>
                {navItems2.map((group, groupIndex) => (
                  <div key={group.group} className={`${SidebarAnimation} mb-6`}>
                    <h2 className="px-4 mb-2 text-xs font-semibold   uppercase tracking-wider">
                      {group.group}
                    </h2>
                    <div className="space-y-1">
                      {group.items.map((item, itemIndex) => (
                        <a
                          key={item.label}
                          href="#"
                          className="flex items-center gap-3 px-4 py-2 text-sm  rounded-lg transition-all duration-200 hover:bg-gradient-to-r hover:from-purple-500/10 hover:to-pink-500/10 hover:text-purple-700 dark:hover:text-purple-300 group"
                        >
            ...
            {sideBarShowing === 3 && (
              <div
                className={`${SidebarAnimation}  flex flex-col pt-4   h-full gap-4 justify-center items-center`}
              >
