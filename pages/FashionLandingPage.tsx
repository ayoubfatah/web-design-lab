...
const images = [
  ...
  {
    src: "https://images.unsplash.com/photo-1512101903502-7eb0c9022c74?w=800&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1yZWxhdGVkfDF8fHxlbnwwfHx8fHw%3D",
    color: "#4e5262",
  },
  {
...
const Carousel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const scrollHandler = () => {
      // Scroll the first container upwards
      scrollPos1 += scrollSpeed;
      if (scrollPos1 >= totalScrollHeight1) {
        scrollPos1 = 0; // Reset to the top of the first set
      }
      container1.scrollTop = scrollPos1;

      // Scroll the second container downwards
      scrollPos2 -= scrollSpeed;
      if (scrollPos2 <= 0) {
        scrollPos2 = totalScrollHeight2; // Reset to the bottom of the first set
      }
